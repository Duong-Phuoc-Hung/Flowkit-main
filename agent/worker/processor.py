...
async def _process_one(req: dict, deferred: dict = None, retry_after: dict = None):
    ...
    try:
        ...
        if _is_error(result):
            ...
            # --- ĐOẠN CODE TỰ ĐỘNG HÓA CỦA HƯNG BẮT ĐẦU TẠI ĐÂY ---
            
            # TỰ ĐỘNG HÓA 1: Nếu vừa xong ảnh Nhân vật -> Tự động nạp lệnh tạo ảnh cho tất cả các Scene
            if req_type == "GENERATE_CHARACTER_IMAGE":
                project_id = req.get("project_id")
                if project_id:
                    logger.info(f"🤖 [Auto-Factory] Nhân vật đã xong. Đang kích hoạt tạo ảnh cho toàn bộ Scene của dự án: {project_id}")
                    # Lấy danh sách các scene thuộc project
                    scenes = await crud.list_scenes(project_id=project_id)
                    for s in scenes:
                        # Nạp yêu cầu tạo ảnh (GENERATE_IMAGE) cho từng scene vào hàng đợi
            ...
            # TỰ ĐỘNG HÓA 1.5: Nếu vừa xong Ảnh Scene -> Tự động nạp lệnh tạo Video cho Scene đó
            elif req_type == "GENERATE_IMAGE":
                logger.info(f"🎞️ [Auto-Factory] Đã có ảnh cho Scene {req.get('scene_id')}. Đang nạp lệnh tạo Video...")
                await crud.create_request({
                    "project_id": req.get("project_id"),
                    "scene_id": req.get("scene_id"),
                    "video_id": req.get("video_id"), # Để sau này còn biết đường concat
                    "type": "GENERATE_VIDEO",
                    "status": "PENDING"
                })

            # TỰ ĐỘNG HÓA 2: Nếu vừa xong Video -> Kiểm tra để tự động Ghép phim (Concat)
            elif req_type == "GENERATE_VIDEO":
                video_id = req.get("video_id")
                if video_id:
                    # Kiểm tra trong DB xem còn Request nào của video này chưa COMPLETED không
                    # (Sử dụng các hàm có sẵn trong crud của bạn)
                    all_reqs = await crud.list_requests(video_id=video_id)
                    unfinished = [r for r in all_reqs if r["status"] != "COMPLETED"]
                    
                    if len(unfinished) == 0:
                        logger.info(f"🎬 [Auto-Factory] Video {video_id} đã hoàn tất toàn bộ clip thô. Đang tự động ghép phim...")
                        try:
                            from agent.services.video_service import concat_video
                            # Chạy concat trong một task riêng để tránh block worker
                            asyncio.create_task(concat_video(video_id))
                        except Exception as e:
                            logger.error(f"❌ Lỗi tự động ghép phim: {e}")
            
            # --- ĐOẠN CODE TỰ ĐỘNG HÓA KẾT THÚC ---
    except Exception as e:
